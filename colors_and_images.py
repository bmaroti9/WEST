...
def blit_pixelart(surface, pos, size):
    global PIXEL_ART
    if PIXEL_ART == 0:
        with open("info.txt", "r") as f:
               info = json.load(f)
        
        width = len(info["picture"])
        height = len(info["picture"][0])
        a = colour_rect = pygame.Surface((width * size, height * size))     
        for y in range(height):
            for x in range(width):
                pygame.draw.rect(a, info["picture"][y][x], Rect(
                x * size, y * size, size, size))   
        PIXEL_ART =  a
    
    surface.blit(PIXEL_ART, pos)
